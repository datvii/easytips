...
const Main = () => {
  ...
  const [cash, setCash] = useState<number>(543);
  ...
  const onChangeHandler = (e: {
    ...
    target: { value: string; };
      ...
      setInpVal(el);
      // +inpVal > +cash ? setValid(false) : setValid(true);
      // setCash(cash - +inpVal);
      isValid(el.length);
      ...
      <div className="evaluation">
        <Container>
          ...
          <div className="evaluation__money">
            <p>{ locale.arrayLang.sum } <b className="evaluation__cash">{ cash }</b> <b>{ locale.arrayLang.currency }</b></p>
              <input 
                type="text"
                name='tips'
          ...
          <div className={`evaluation__btns ${ !checked || !inpVal ? 'inactive' : '' }`}>
            {(device === 'ios' || device === 'android') && <a href="/" onClick={(e) => payment(e, device) } className="btn btn--white">{ locale.arrayLang.pay } {device === 'ios' ? <img src={ aPay } alt="apple pay"/> : <img src={ gPay } alt="google pay"/>}</a>}
